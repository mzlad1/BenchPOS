...
window.t = function(key, args) {
    ...
    const processTemplate = (text, args) => {
        if (!args || typeof args !== 'object' || typeof text !== 'string') return text;
...
const i18n = {
    ...
    async init() {
        ...
        try {
            ...
            console.log(`Translation keys loaded: ${Object.keys(this.translations[this.currentLanguage]).length}`);
                ...
                if (window.LayoutManager) {
                    ...
                    if (statusText) {
                        ...
                        statusText.textContent = isOnline ?
                            window.t("header.onlineMode") : window.t("header.offlineMode");
                    }
                }
            };
    ...
    async loadAllTranslations() {
        ...
        loadPromises.push(this.loadTranslationsFromMultipleSources(this.currentLanguage));
    ...
    async loadTranslationsFromMultipleSources(lang) {
        ...
        if (cached) {
            ...
            if (this.debug) console.log(`Loaded ${Object.keys(cached).length} translations for ${lang} from cache`);
            return true;
        }
        ...
        for (const path of possiblePaths) {
            try {
                ...
                if (this.debug) console.log(`Successfully loaded ${Object.keys(flattened).length} translations for ${lang} from ${path}`);
                return true;
            } catch (error) {
                if (this.debug) console.warn(`Failed to load ${lang} from ${path}:`, error);
            }
        }

        console.warn(`Could not load additional translations for ${lang}, using core translations only`);
        return false;
    },
    ...
    async changeLanguage(lang) {
        ...
        try {
            ...
            if (Object.keys(this.translations[lang]).length <= Object.keys(CORE_TRANSLATIONS[lang]).length) {

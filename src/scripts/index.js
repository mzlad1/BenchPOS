...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        document.getElementById("sync-button-bottom").addEventListener("click", syncData);
        ...
        document.getElementById("inventory-card").addEventListener("click", (event) => {
            if (!document.getElementById("inventory-card").classList.contains("disabled")) {
        ...
        document.getElementById("reports-card").addEventListener("click", (event) => {
            if (!document.getElementById("reports-card").classList.contains("disabled")) {
...
async function fetchDashboardData() {
    try {
        ...
        document.getElementById("products-count").textContent = cachedProducts.length;

        // Count low stock items (stock < 10)
        const lowStockItems = cachedProducts.filter(product => product.stock < 10).length;
        document.getElementById("low-stock-count").textContent = lowStockItems;
        document.getElementById("inventory-badge").textContent = lowStockItems > 0 ? lowStockItems : '';
        ...
        const totalRevenue = cachedInvoices.reduce((sum, invoice) => sum + (invoice.total || 0), 0);
...
async function syncData() {
    try {
        ...
        syncButtons.forEach(button => {
            ...
            button.innerHTML = '<div class="btn-icon" style="animation: spin 1s linear infinite;">🔄</div><span>Syncing...</span>';
        });
...
async function loadRecentActivity() {
    try {
        ...
        if (window.activityLogger && typeof window.activityLogger.getAll === 'function') {
            ...
            console.log(`Loaded ${activityLogs.length} activities from global logger`);
        } else {
            // Legacy fallback
            try {
                ...
                if (storedLogs) {
                    ...
                    console.log(`Loaded ${activityLogs.length} activities from localStorage directly`);
                }
            } catch (error) {
        ...
        if (window.api && typeof window.api.getActivityLogs === 'function') {
            try {
                ...
                const existingTimestamps = new Set(activityLogs.map(log => log.timestamp));
        ...
        const lastSyncTime = window.api.getLastSyncTime ? await window.api.getLastSyncTime() : null;
        if (lastSyncTime) {
            activities.push({
                type: 'sync',
                icon: '🔄',
            ...
            switch(log.action) {
                case 'update':
                    activities.push({
                        type: 'update',
                        icon: '✏️',
                        text: `Updated <strong>${log.itemName || log.itemType || 'item'}</strong>`,
                ...
                case 'delete':
                    activities.push({
                        type: 'delete',
                        icon: '🗑️',
                        text: `Deleted <strong>${log.itemName || log.itemType || 'item'}</strong>`,
                        timestamp: new Date(log.timestamp || Date.now()),
                        badge: 'Delete',
                ...
                case 'add':
                    activities.push({
                        type: 'add',
                        icon: '✚',
                        text: `Added <strong>${log.itemName || log.itemType || 'item'}</strong>`,
                        timestamp: new Date(log.timestamp || Date.now()),
                        badge: 'New',
                        badgeClass: 'badge-success'
                    });
...
function processChartData(invoices) {
    ...
    if (currentChartPeriod === 'week') {
        ...
        filteredInvoices.forEach(invoice => {
            ...
            const productCount = invoice.items ? invoice.items.reduce((sum, item) => sum + (item.quantity || 0), 0) : 0;
        ...
        filteredInvoices.forEach(invoice => {
            ...
            const chunk = `${monthName} ${dayChunk+1}-${Math.min(dayChunk+3, 31)}`;
...
async function createAndUpdateChart() {
    try {
        ...
        const chartWrapper = document.getElementById('chart-wrapper');
        chartWrapper.innerHTML = '<div class="chart-error">Error loading chart data</div>';
    }
}
...
async function updateSalesChart() {
    try {
        ...
        if (currentChartView === 'revenue') {
            ...
            salesChart.data.datasets[0].borderColor = getComputedStyle(document.body).getPropertyValue('--primary');
            ...
            salesChart.data.datasets[0].borderColor = getComputedStyle(document.body).getPropertyValue('--secondary');
            salesChart.data.datasets[0].backgroundColor = 'rgba(14, 165, 233, 0.2)';
        } else {
            salesChart.data.datasets[0].label = 'Products Sold';
            salesChart.data.datasets[0].data = sortedData.map(data => data.products);
            salesChart.data.datasets[0].borderColor = getComputedStyle(document.body).getPropertyValue('--accent');

...
function initUserManager() {
    ...
    if (window.api && window.api.ipcRenderer) {
        ...
        showNotification('Application error: IPC communication not available', 'error');
        return;
    }
    ...
    checkAuth().then(() => {
        ...
        showNotification('You must be logged in to access user management.', 'error');
...
async function fetchUsers() {
    ...
    `;
        ...
        if (currentFilters.search && currentFilters.search.length > 0) {
            ...
            console.log(`Filtered to ${filteredUsers.length} users after client-side search for "${searchTerm}"`);
        }
...
function renderUsers() {
    ...
    usersToDisplay.forEach(user => {
      ...
      <td><span class="user-role ${user.role || 'unknown'}">${user.role || 'Unknown'}</span></td>
      <td><span class="user-status ${user.status || 'inactive'}">${user.status || 'Inactive'}</span></td>
      <td>${lastLoginText}</td>
      <td>
        <div class="action-buttons">
          <button class="action-btn edit-btn" data-id="${user.id}" title="Edit">✏️</button>
          <button class="action-btn delete-btn" data-id="${user.id}" title="Delete">🗑️</button>
        </div>
      </td>
    `;

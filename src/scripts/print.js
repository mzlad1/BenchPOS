...
window.completeSale = async function() {
    ...
    try {
        ...
        if (typeof document.getElementById("discount-value") !== 'undefined' &&
        ...
        // Show notification
        showToastNotification(`${invoiceData.isRefund ? 'Return' : 'Sale'} completed successfully. Press F8 to print receipt.`);
...
window.printReceipt = function() {
    ...
    const debugInfo = {
        ...
        allInvoicesLength: typeof window.allInvoices !== 'undefined' ? window.allInvoices.length : 0,
    ...
    try {
        ...
        if (window.isViewingInvoice === true &&
        ...
        if (typeof window.cart === 'object' &&
            ...
            console.log("Generating receipt from current cart items:", window.cart.length, "items");
            ...
            const invoiceData = {
                ...
                customer: typeof window.customerNameEl !== 'undefined' && window.customerNameEl ?
                    window.customerNameEl.value || "Guest Customer" : "Guest Customer",
                subtotal: typeof window.subtotalEl !== 'undefined' && window.subtotalEl ?
                    parseFloat(window.subtotalEl.textContent.replace("$", "")) :
                    window.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0),
                tax: typeof window.taxEl !== 'undefined' && window.taxEl ?
                    parseFloat(window.taxEl.textContent.replace("$", "")) : 0,
                total: typeof window.totalEl !== 'undefined' && window.totalEl ?
        ...
        console.warn("Nothing to print: No invoice being viewed, empty cart, and no receipt container");
        ...
        try {
            if (typeof downloadReceiptAsHTML === 'function') {
                ...
                const receiptContainer = document.getElementById('receipt-container');
                if (receiptContainer && receiptContainer.innerHTML.trim() !== '') {
                    downloadReceiptAsHTML(receiptContainer.innerHTML, styles, "receipt-" + new Date().getTime());
                    return;
                }

                const hiddenContainer = document.getElementById('hidden-receipt-container');
                if (hiddenContainer && hiddenContainer.innerHTML.trim() !== '') {
                    downloadReceiptAsHTML(hiddenContainer.innerHTML, styles, "receipt-" + new Date().getTime());
                    return;
                }

                alert("No content to print. Please add items to the cart first.");
            } else {
                alert("Printing failed and fallback download not available. Please try again later.");
            }
        } catch (fallbackError) {
            console.error("Fallback printing also failed:", fallbackError);
            alert("Unable to print or download receipt. Please check your system configuration.");
        }
    }
};
...
function downloadReceiptAsHTML(html, styles, invoiceId) {
    try {
        ...
        showToastNotification(`Receipt downloaded as HTML file: receipt-${invoiceId}.html`);
    } catch (error) {
...
function enhanceF8ShortcutHandler() {
    ...
    }, true); // true = capturing phase, ensures this runs first
    ...
    if (typeof window.functionKeyHandlers !== 'undefined' && window.functionKeyHandlers.F8) {
...
function initDirectCartPrinting() {
    ...
    showToastNotification("Receipt system improved. F8 will print the correct content.");
}

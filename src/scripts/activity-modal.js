...
function setupActivityModal() {
  ...
  const styleExists = document.querySelector(
  ...
  if (nextButton) {
    nextButton.addEventListener("click", function () {
      const totalPages = Math.ceil(
        filteredActivities.length / activityModalPageSize
      );
...
async function loadFullActivityList() {
  ...
  try {
      ...
      <div class="activity-item loading-item">
        <div class="activity-content">
          <div class="activity-text">${window.t(
            "dashboard.activity.modal.loading",
            "Loading activities..."
          )}</div>
        </div>
      </div>
    ...
    if (
      ...
      console.log(
        `Loaded ${allActivities.length} activities from global logger`
      );
    } else {
      ...
      try {
        ...
        if (storedLogs) {
          ...
          console.log(
            `Loaded ${allActivities.length} activities from localStorage`
          );
        }
      } catch (error) {
    ...
    console.log(
      `Adding activities from ${invoices.length} invoices and ${products.length} products`
    );
    ...
    products
      ...
      .forEach((product) => {
        allActivities.push({
          ...
          itemType: "product",
          itemName:
            product.name || window.t("dashboard.activity.unknown", "Unknown"),
    ...
    if (typeValue !== "all") {
      allActivities = allActivities.filter(
        (activity) => activity.action === typeValue
      );
    }
    ...
    if (activityList) {
      activityList.innerHTML = `
        <div class="activity-item error-item">
          <div class="activity-content">
            <div class="activity-text">${window.t(
              "dashboard.activity.modal.error",
...
function renderActivityModalPage() {
  ...
  const endIndex = Math.min(
    startIndex + activityModalPageSize,
    filteredActivities.length
  );
  const currentPageActivities = filteredActivities.slice(startIndex, endIndex);

  console.log(
    `Displaying activities ${startIndex + 1} to ${endIndex} of ${
      filteredActivities.length
    }`
  );
  ...
  if (filteredActivities.length === 0) {
    activityList.innerHTML = `
      <div class="no-activities">
        <p>${window.t(
          "dashboard.activity.modal.noActivities",
    ...
    currentPageActivities.forEach((activity, index) => {
      ...
      let html = `
        <div class="activity-icon">${
          activity.icon || getIconForAction(activity.action)
        }</div>
        <div class="activity-content">
          <div class="activity-text">${
            activity.text || getTextForActivity(activity)
          }</div>
      ...
      if (activity.badge) {
        html += `<div class="activity-badge ${
          activity.badgeClass || getBadgeClassForAction(activity.action)
        }">${activity.badge || activity.action}</div>`;
      }
  ...
  switch (activity.action) {
    ...
    case "sale":
      if (activity.itemName) {
        ...
        return window.t(
          "dashboard.activity.saleCompleted",
          "New sale completed"
        );
      }
    case "stock":
      return translateTemplate(
        ...
        "Low stock alert for <strong>{name}</strong>",
        {
          name:
            activity.itemName ||
            window.t("dashboard.activity.product", "product"),
        }
      );
    default:
      return translateTemplate(
        ...
        "{action} for {name}",
        {
          action:
            activity.action ||
            window.t("dashboard.activity.activity", "Activity"),
          name:
            activity.itemName ||
            activity.itemType ||
            window.t("dashboard.activity.item", "item"),
